@extends('layouts.app')

@section('content')
    <div class="home-pg container w-80">
        <div class="content h-100 d-flex flex-column justify-content-center align-items-center text-center px-4">
            <h1 class="h2">
                Que tal organizar seus <span class="highlight-title">projetos</span> de forma simples e eficiente?
            </h1>
            <p>
                Escopar é uma ferramenta que te ajuda a organizar seus projetos de forma simples e eficiente.
            </p>
            <a href="#" class="btn btn-salmon-light">Começar</a>
        </div>
    </div>
@endsection
